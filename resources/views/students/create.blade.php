...
@section('title', 'Register New Student - StudentMS')
    ...
    <div class="glass-card rounded-3xl overflow-hidden shadow-xl border border-slate-800">
        ...
        <div class="px-8 py-6 bg-slate-900/40 border-b border-slate-800/80">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-xl font-bold text-white">Register Student</h2>
                    <p class="text-xs text-slate-400 mt-0.5">Fill out all fields below to register a new student to the database.</p>
                </div>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('students.store') }}" class="p-8 space-y-6">
            ...
            <div class="space-y-2">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="John Doe" required class="w-full pl-10 pr-4 py-3 rounded-xl input-glass text-sm focus:ring-2 focus:ring-indigo-500/20 @error('name') border-rose-500/80 focus:border-rose-500 @enderror" />
                </div>
                @error('name')
            ...
            <div class="space-y-2">
                ...
                <div class="relative">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email') }}" placeholder="john.doe@example.com" required class="w-full pl-10 pr-4 py-3 rounded-xl input-glass text-sm focus:ring-2 focus:ring-indigo-500/20 @error('email') border-rose-500/80 focus:border-rose-500 @enderror" />
                </div>
                @error('email')
                    <p class="text-xs font-semibold text-rose-400 flex items-center gap-1.5 mt-1">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
            ...
            <div class="space-y-2">
                ...
                <div class="relative">
                    ...
                    <input type="number" name="age" id="age" value="{{ old('age') }}" min="5" max="120" placeholder="21" required class="w-full pl-10 pr-4 py-3 rounded-xl input-glass text-sm focus:ring-2 focus:ring-indigo-500/20 @error('age') border-rose-500/80 focus:border-rose-500 @enderror" />
                </div>
                @error('age')
                    <p class="text-xs font-semibold text-rose-400 flex items-center gap-1.5 mt-1">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
